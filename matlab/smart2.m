...
% xlabel('Time (s)');
% ylabel('Value');
% distance_error_plot = plot(ax, NaN, NaN, 'DisplayName', 'Data 1'); % Initial empty plot for Data 1
% hold on;
% heading_error_plot = plot(ax, NaN, NaN, 'DisplayName', 'Data 2'); % Initial empty plot for Data 2
% axis([0 100 0 100]); % Set limits for both axes
% legend;
...
for i=1:Total_Steps
    ...
    % heading_errors(i) = heading_error;
    % time_steps(i) = i;

    % set(distance_error_plot, 'XData', time_steps, 'YData', distance_errors);
    % set(heading_error_plot, 'XData', time_steps, 'YData', heading_errors);
    % drawnow;
    ...
    elseif REACHED_HEADING
        ...
        if abs(heading_error) > deg2rad(HEADING_THRESHOLD_DEGREES)
            if mod(heading_error,deg2rad(360)) < deg2rad(180)
                disp("    Turning - Right")
